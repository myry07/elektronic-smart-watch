...
void lcdInit(TFT_t *dev, int width, int height, int offsetx, int offsety, DIRECTION dir)
{
	...
	spi_master_write_data_byte(dev, madctl); // 方向控制
...
// 可用于 LVGL 刷新的通用区域刷新函数
void lcdDrawBitmap(TFT_t *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, const uint16_t *color)
{
    ...
    spi_master_write_addr(dev, x1 + dev->_offsetx, x2 + dev->_offsetx);
    spi_master_write_command(dev, 0x2B);
    spi_master_write_addr(dev, y1 + dev->_offsety, y2 + dev->_offsety);
    spi_master_write_command(dev, 0x2C);

    while (size > 0)
    {
        uint16_t send_size = size > 512 ? 512 : size;
        spi_master_write_colors(dev, (uint16_t *)color, send_size);
        color += send_size;
        size -= send_size;
    }
}
